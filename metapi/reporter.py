    ...
    for i in df.index.unique():
        if (not df.loc[i, ].query('reads=="fq1" and step=="rmhost"').empty) and \
           (not df.loc[i, ].query('reads=="fq1" and step=="trimming"').empty):
            reads_num_rmhost = df.loc[i, ]\
                                 .query('reads=="fq1" and step=="rmhost"')["num_seqs"][0]
            reads_num_trimming = df.loc[i, ]\
                                   .query('reads=="fq1" and step=="trimming"')["num_seqs"][0]
...
def main():
    ...
    trimming_list = pd.read_csv(args.trimming_list, header=None, names=["trimming"])
    rmhost_list = pd.read_csv(args.rmhost_list, header=None, names=["rmhost"])
