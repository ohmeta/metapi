rule checkv_download_db:
    ...
    log:
        os.path.join(config["output"]["check"], "logs/checkv_download_db/checkv_download_db.log")
    benchmark:
        os.path.join(config["output"]["check"], "benchmark/checkv_download_db/checkv_download_db.txt")
    params:
...
rule checkv:
    ...
    log:
        os.path.join(
            config["output"]["check"],
            "logs/checkv/{binning_group}.{assembly_group}.{assembler}.{identifier}.log")
    benchmark:
        os.path.join(
            config["output"]["check"],
            "benchmark/{binning_group}.{assembly_group}.{assembler}.{identifier}.txt")
    params:
        db = config["params"]["checkv"]["db"],
