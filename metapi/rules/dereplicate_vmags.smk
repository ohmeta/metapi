...
rule dereplicate_vmags_build_db:
    ...
    log:
        os.path.join(
            config["output"]["dereplicate"],
            "logs/dereplicate_vmags_build_db/{assembler}.log")
    benchmark:
        os.path.join(
            config["output"]["dereplicate"],
            "benchmark/dereplicate_vmags_build_db/{assembler}.txt")
    params:
        db = os.path.join(
            config["output"]["dereplicate"],
...
rule dereplicate_vmags_blastn:
    ...
    log:
        os.path.join(
            config["output"]["dereplicate"],
            "logs/dereplicate_vmags_blastn/{assembler}.log")
    benchmark:
        os.path.join(
            config["output"]["dereplicate"],
            "benchmark/dereplicate_vmags_blastn/{assembler}.txt")
    params:
        db = os.path.join(
            config["output"]["dereplicate"],
            "genomes/virome/blastdb/vMAGs_hmq.{assembler}.blastdb"),
...
rule dereplicate_vmags_compute_ani:
    ...
    log:
        os.path.join(
            config["output"]["dereplicate"],
            "logs/dereplicate_vmags_compute_ani/{assembler}.log")
    benchmark:
        os.path.join(
            config["output"]["dereplicate"],
            "benchmark/dereplicate_vmags_compute_ani/{assembler}.txt")
    params:
...
rule dereplicate_vmags_clust:
    ...
    log:
        os.path.join(
            config["output"]["dereplicate"],
            "logs/dereplicate_vmags_clust/{assembler}.log")
    benchmark:
        os.path.join(
            config["output"]["dereplicate"],
            "benchmark/dereplicate_vmags_clust/{assembler}.txt")
    params:
...
rule dereplicate_vmags_postprocess:
    ...
    log:
        os.path.join(
            config["output"]["dereplicate"],
            "logs/dereplicate_vmags_postprocess/{assembler}.log")
    benchmark:
        os.path.join(
            config["output"]["dereplicate"],
            "benchmark/dereplicate_vmags_postprocess/{assembler}.txt")
    shell:
        '''
...
if config["params"]["dereplicate_vmags"]["blast"]["do"] and config["params"]["checkv"]["do"]:
