...
def main():
    parser = argparse.ArgumentParser(description="HUMAnN3 postprocess wrapper")
    subparsers = parser.add_subparsers(title="available subcommands", metavar="")

    parser_regroup_table = subparsers.add_parser(
        "regroup_table", prog="humann3_postprocess_wrapper.py regroup_table"
    )
    ...
    parser_join_tables = subparsers.add_parser(
        "join_tables", prog="humann3_postprocess_wrapper.py join_tables"
    )
    ...
    parser_split_stratified_table = subparsers.add_parser(
        "split_stratified_table", prog="humann3_postprocess_wrapper.py split_stratified_table"
    )
