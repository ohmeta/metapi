...
class RecordMatrix(object):
    ...
    def addSamRecord(self, readName, samRecord):
        if(samRecord.isFirstRead()):
            self.__matrix["count1"][readName] = self.__matrix["count1"].get(
                readName, 0) + 1
            if self.__matrix["count1"][readName] > 1:
                return
            self.__matrix["record1"].setdefault(readName, []).append(samRecord)
        else:
            self.__matrix["count2"][readName] = self.__matrix["count2"].get(
                readName, 0) + 1
...
class Sample(object):
    ...
    def __SamLineFilter(self, line):
        ...
        try:
            ...
            tmpLen = sum([int(i)
                          for i in re.findall(r"(\d+)(?:M|I|D)", tmp[5])])
        except Exception as e:
            raise e
        if((1-tmpNM/tmpLen) < self.__identity):
            return None
    ...
    def __SingleAlignAdd(self, rec, ins, discordant=False):
        ...
        if discordant:
            self.__abunMatrix["samrec_list"].setdefault(
                _markerName, []).append(rec)
            return True
        if(rec.isReverse()):
            if((self.markerMatrix.getMarkerLength(_markerName)-rec.getFirstLoc()) < (ins+100)):
                self.__abunMatrix["frag_num"][_markerName] = self.__abunMatrix["frag_num"].get(
                    _markerName, 0) + 1
                self.__abunMatrix["samrec_list"].setdefault(
                    _markerName, []).append(rec)
                return True
    ...
    def __PairAlignAdd(self, rec1, rec2, ins):
        ...
        if (rec1.isProperlyAlignPair()):
            self.__abunMatrix["frag_num"][_markerName] = self.__abunMatrix["frag_num"].get(
                _markerName, 0) + 1
            self.__abunMatrix["samrec_list"].setdefault(
                _markerName, []).append(rec1)
            self.__abunMatrix["samrec_list"].setdefault(
                _markerName, []).append(rec2)
        else:
            discordant = False
    ...
    def AbunCalculate(self):
        for _marker in self.markerMatrix.getMarkerIterator():
            self.__totalAbundance += self.__abunMatrix["frag_num"].get(
                _marker, 0)/self.markerMatrix.getMarkerLength(_marker)
            self.__totalReads += self.__abunMatrix["frag_num"].get(_marker, 0)
            self.__abunMatrix["abundance"][_marker] = self.__abunMatrix["frag_num"].get(
                _marker, 0)/self.markerMatrix.getMarkerLength(_marker)
        ...
        try:
            ...
            if (outputType == "evaluation") or (outputType == "all"):
                with open(os.path.join(prefix, self.__sampleName + ".evaluation.abundance" + suffix), "w") as abunOut:
                    abunOut.write(
                        "marker,reads(pair) number,relative abundance,reads list,\n")
                    for _marker in self.__abunMatrix["frag_num"].keys():
                        abunOut.write("{marker},{frag_num},{rel_abun},{reads_list},\n".format(
                            ...
                            rel_abun=self.__abunMatrix["abundance"][_marker] /
                            self.__totalAbundance,
                            reads_list="|".join(
                                [str(rec) for rec in self.__abunMatrix["samrec_list"][_marker]])
                        ))
            ...
            # ID    reads_pairs reads_proportion    gene_abundance
                ...
                with open(os.path.join(prefix, self.__sampleName + ".abundance" + suffix), "w") as abunOut:
                    ...
                    for _marker in self.markerMatrix.getMarkerIterator():
                        abunOut.write("{markerID}\t{reads_num}\t{reads_prop}\t{rel_abun}\n".format(
                            ...
                            reads_num=self.__abunMatrix["frag_num"].get(
                                _marker, 0),
                            reads_prop=self.__abunMatrix["frag_num"].get(
                                _marker, 0)/self.__totalReads,
                            rel_abun=self.__abunMatrix["abundance"].get(
                                _marker, 0)/self.__totalAbundance
                        ))
                ...
                with open(os.path.join(prefix, self.__sampleName + ".non_zero.abundance" + suffix), "w") as abunOut:
                    ...
                    for _marker in self.markerMatrix.getMarkerIterator():
                        ...
                        abunOut.write("{markerID}\t{reads_num}\t{reads_prop}\t{rel_abun}\n".format(
                            markerID=self.markerMatrix.getMarkerID(_marker),
                            redes_num=self.__abunMatrix["frag_num"].get(
                                _marker, 0),
                            reads_prop=self.__abunMatrix["frag_num"].get(
                                _marker, 0)/self.__totalReads,
                            rel_abun=self.__abunMatrix["abundance"].get(
