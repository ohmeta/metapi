...
def merge_more2kb_contigs_rename(contigs_list, merge_out):
    with open(merge_out, 'w') as out:
        with open(contigs_list, 'r') as contigs_handle:
            for contigs in contigs_handle:
                sample_name = os.path.basename(contigs).split(".")[0]
                with open(contigs.strip(), 'r') as contigs_fa:
                    for record in  SeqIO.parse(contigs_fa,  'fasta'):
                        if len(record.seq) >= 2000:
                            record.description = record.description + "\t" + sample_name
                            SeqIO.write(record, out, 'fasta')
...
def main():
    ...
    merge_more2kb_contigs_rename(args.contigs_list, args.merge_out)
